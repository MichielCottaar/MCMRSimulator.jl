module Fix
import LinearAlgebra: I, transpose, cross, norm, ⋅
...
function fix_type(bendy_cylinder::BendyCylinder, index::Int, original_index::Int; kwargs...)
    ...
    nadd = all(iszero.(bendy_cylinder.nclosed.value)) ? 0 : div(bendy_cylinder.spline_order.value, length(bendy_cylinder.control_points), RoundUp)
    if iszero(nadd)
        full_control_points = SVector{3}.(control_points)
        full_radii = Float64.(radii)
    else
        @assert !isnothing(bendy_cylinder.repeats.value)
        full_control_points = [SVector{3}(cp .+ bendy_cylinder.repeats.value .* bendy_cylinder.nclosed.value .* index_add) for index_add in -nadd:nadd for cp in control_points]
    ...
    return fix_type(Mesh(; mesh_kwargs...), index, original_index; kwargs...)
end
