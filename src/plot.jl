...
Instead they are implemented in the `MakieMCMRSimulator` extension if [`Makie`](https://makie.org) is installed and imported.
...
If a [`PlotPlane`](@ref) is provided the projection on it will be plotted. Otherwise, the 3-dimensional geometry will be plotted.
...
"""
    plot_geometry!(scene, [plot_plane], geometry)

Plots the geometry on an existing plot scene.
If a [`PlotPlane`](@ref) is provided the projection on it will be plotted. Otherwise, the 3-dimensional geometry will be plotted.
...
Plots the off-resonance field generated by the geometry in the [`PlotPlane`](@ref) in a new plot. 

When overlaying both the off-resonance field and the geometry, call this function first before calling [`plot_geometry!`](@ref).
...
"""
    plot_off_resonance!(scene, plot_plane, geometry)

Plots the off-resonance field generated by the geometry in the [`PlotPlane`](@ref) on an existing scene. 
...
"""
    plot_sequence!(scene, sequence)

Plots the sequence on an existing plot scene.
...
Additional keywords are passed on to `Makie.meshscatter`.
...
"""
    plot_snapshot!([plot_plane], snapshot; kind=:scatter)

Plots a [`Snapshot`](@ref) on an existing plot scene. 
