"""
    Affine(obstructions::Obstructions, transform::CoordinateTransformations.Transformation)

Underlying `obstructions` are linearly transformed (e.g., rotated or shifted) using the given `transform`.
"""
struct Affine{N, O, T <: CoordinateTransformations.Transformation} <: TransformObstruction
