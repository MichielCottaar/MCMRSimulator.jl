@testset "test_collisions.jl" begin
    ...
    @testset "Reflections on planes of cylinders" begin
        ...
        @testset "Two overlapping spheres" begin
            ...
            @test all(length(isin) >= 1 for isin in mr.isinside(mr.fix(base_spheres), overlap_final))
            ...
            @test all(length(isin) == 0 for isin in mr.isinside(mr.fix(base_spheres), outside_final))
