@testset "Generate and apply microstructural fields" begin
    for step_size in (1., 0.3, 0.123)
        @testset "Simulate empty environment/sequence with varying step sizes" begin
            orient = mr.evolve_to_time(mr.MultiSpin(1, transverse=1.), 0., 1., mr.Microstructure(), step_size).orientations[1]
            @test mr.phase(orient) == 0.
        end
        @testset "Test constant off-resonance field" begin
            orient = mr.evolve_to_time(mr.MultiSpin(1, transverse=1.), 0., 0.3, mr.Microstructure(off_resonance=mr.field(2.)), step_size).orientations[1]
        ...
        @testset "Test gradient off-resonance field" begin
            micro = mr.Microstructure(off_resonance=mr.field(SA_F64[1.5, 0., 0.], 2.))
            spin = mr.evolve_to_time(mr.MultiSpin(1, transverse=1.), 0., 0.3, micro, step_size)
            ...
            orient = mr.evolve_to_time(mr.MultiSpin(1, transverse=1., position=SA_F64[3., 0., 0.]), 0., 0.2, micro, step_size).orientations[1]
