...
if docker_tag_exists $tag-arm64 ; then
    echo "Checking out tag '$tag' from git"
    git checkout $tag
    echo "Building $tag-amd64 locally"
    docker login
    docker build -t michielc/mcmr:$tag-amd64 .
    docker push michielc/mcmr:$tag-amd64
    docker manifest create michielc/mcmr:$tag michielc/mcmr:$tag-arm64 michielc/mcmr:$tag-amd64
